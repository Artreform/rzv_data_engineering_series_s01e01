...
@dag(default_args = default_args, description = 'Pipeline to load from OLTP sources to DWH(Staging)', schedule_interval='*/10 * * * *', catchup=False)
def load_staging_data():
    ...
    def extract_data(table, conn_id, **context):
        ...
        logging.info(context["dag_run"].execution_date)
        ...
        try:
            ...
            max_load_date = None
            logging.info(f"First load of {table} table")
        logging.info(f"Got max({increment_col}) from target {table} table: {max_load_date}")
        
        if not max_load_date:
            sql = f"""select * from {table};"""
            logging.debug(sql)
        else:
            sql = f"""select * from {table} where {increment_col} > '{max_load_date}'::timestamp - interval '600 seconds';""" #почему должен быть определенный интервал, изменения могут внести в любую строку?
            logging.debug(sql)
        ...
        logging.info(f"Data extracted {df.shape[0]} rows, {df.shape[1]} columns, from source {conn_id} with {max_load_date} to {file_path}")
    ...
    def transform(file_path, table, conn_id, **context):
         ...
         df.to_csv(file_path, sep=';', header=True, index=False, mode='w', encoding='utf-8')
         logging.info(f"Data transformed {df.shape[0]} rows, {df.shape[1]} columns, from source {conn_id} to {file_path}")
    ...
    def load(file_path, table):
         ...
         logging.info(f"Data loaded {df.shape[0]} rows, {df.shape[1]} columns, into staging.{table} from {file_path}")
